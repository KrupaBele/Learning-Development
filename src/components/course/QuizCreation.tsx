...
const QuizCreation = ({
  ...
  const formattedQuizzes = quizzes.map((quiz) => ({
    ...quiz,
    correctAnswers:
      quiz.correctAnswers || quiz.answer.map((ans) => String(ans)), // Convert answer to correctAnswers
  }));
  ...
  const runAiGenerate = async (mode: "replace" | "append") => {
    if (!chapters.length) {
      toast.error(
        "Add chapters first — the AI uses chapter text to build questions.",
      );
      return;
    }
    if (
    ...
    try {
      ...
      const next = mode === "replace" ? generated : [...quizList, ...generated];
      setQuizList(next);
      onUpdate(next);
      toast.success(
        mode === "replace"
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Generate draft questions from your chapter text, or build
              everything yourself — you can mix both. AI questions are tagged
              with the chapter they came from so you can place them after the
              right chapter on the Course order tab.
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {quizList.map((quiz) => (
          <div
            key={String(quiz._id ?? quiz.id)}
            className="bg-white dark:bg-dark-800 p-6 rounded-lg border border-gray-200 dark:border-dark-700 shadow-sm"
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <button
                onClick={() => handleRemoveQuestion(quiz._id ?? quiz.id)}
